...
- **cases/** → `IndustryList` (string[]): индустрия клиента. Источник: WP-таксономия `case_industry`. 10 значений: DIY, FMCG, HoReCa, Ритейл, Бьюти, Детские товары, Искусство, Образование, Строительство, 18+. Пустой массив `[]` если не заполнено в WP. `AuthorIdList` может быть `null` (для кейсов-подрядчиков).
...
**Извлечение составных блоков:**

- `_extract_content_bg()` — детектирует контейнеры с `background_background` в `data-settings` + кнопка внутри → `big_content_bg` (с image) или `small_content_bg` (без image). Guard: пропуск quote.svg, ≤ 4 виджетов. **Важно:** при массовом вызове через `find_all` использовать CSS-селекторы (`tag.select()`), а не `find_all(lambda)` — BS4 bug с вложенными lambda-итераторами.
- `_extract_elementor_quote()` — цитаты с quote.svg
- `_extract_factoid()` — выделенные блоки
- `heading.default` — атомарная обработка (маркер `_separate`, не мерджится в `_merge_micro_text_blocks`)
...
- `convert_big_content_bg()` — `big_content_bg` → `BigContentBg` (ImageUrl, Alt, Elements, BtnText, BtnUrl)
- `convert_small_content_bg()` — `small_content_bg` → `SmallContentBg` (TitleElements, Elements, BtnText, BtnUrl)
